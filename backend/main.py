...
async def chat_with_ai(request: ChatRequest):
    ...
    try:
        ...
        # 4턴이거나 추가 정보가 필요 없으면 종료
        should_end = user_turns == 4 or not required_info
        medical_brief = None
...
def llm_infer_treatment_and_missing_info(docs, scores, current_state, threshold, solar_model):
    try:
        ...
        위 정보를 바탕으로 다음 형식에 맞춰 응답해줘:

        🚨 [응급처치 방법]
        - (환자 상태에 따른 즉시 시행해야 할 처치를 순서대로 나열 간단하고 길지 않게 약 5줄 정도도)
        - (각 처치의 구체적인 방법과 주의사항)

        📋 [추가로 필요한 정보]
        - (환자 상태 파악을 위해 추가로 확인이 필요한 정보들 간단하게)
        - (각 정보가 왜 필요한지 간단히 설명)
        ...
        [추가로 필요한 정보]가 필요 없다면 "추가 정보 필요 없음"이라고 반드시 명시해줘.
        [추가로 필요한 정보]가 있다면 "추가 정보 필요 없음"을 명시하지 말아줘 제발발
        각 섹션은 bullet point(-)로 시작하는 항목들로 구성해주세요.
        """
